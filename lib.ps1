function Get-Splunk(){
    ...
    msiexec.exe /i $out AGREETOLICENSE=Yes SPLUNKUSERNAME=admin SPLUNKPASSWORD=admin /quiet
}
...
#adding new group policy
New-GPO -Name $gpo | New-GPLink -Target "dc=hk,dc=local" -LinkEnabled Yes
...
Set-GPRegistryValue -Name $gpo -Key $srv -ValueName "DelayedAutostart" -Type DWord -Value 2
...
#Autostart WinRM
Set-GPRegistryValue -Name $gpo -Key $srv -ValueName "Start" -Type DWord -Value 0
